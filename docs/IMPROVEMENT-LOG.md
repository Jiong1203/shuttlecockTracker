...
| 二 | C3 紀錄截斷提示 | ✅ | `feat/audit-wave-2`（含邊界修正；資料未達上限故未實機驗證） |
...
### 2026-09-04 · 實機驗證完成與截斷判斷的邊界修正

**驗證結果**：22 項檢查完成 21 項。唯一未驗證的是領取紀錄的截斷提示（C3），
因為實際資料未達 100 筆上限，該提示本就不該出現。過程中回報並修正了四個問題：
CSV 欄位型別、名冊排序、不存在的球隊、活動詳情的無盡載入。

**順帶修正的邊界瑕疵**：截斷提示原本以 `records.length >= LIMIT` 判斷，
但 API 也只取 `LIMIT` 筆——**資料剛好等於上限時無法分辨「剛好滿」與「被截斷」**，
會誤報「更早的紀錄仍保存於系統中」。這正好落在無法實機驗證的那一項上。

改為 API 取 `LIMIT + 1` 筆，回傳筆數超過上限即代表確實還有更早的紀錄；
前端只呈現前 `LIMIT` 筆，搜尋、月份選項與空狀態判斷一併改用截斷後的清單。
領取紀錄與入庫紀錄兩處皆同。
