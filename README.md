...
- **智能數據結算**：可按多位領取人、多種球種、時間區間即時試算總領取桶數與應付金額（FIFO 計算，球種與領取人均支援多選）。
- **歷史紀錄管理**：清楚記錄每一筆變動，並支援紀錄刪除與庫存連動更新。
- **團體設定管理**：支援修改球團名稱、聯絡信箱、系統登入密碼以及入庫管理密碼。
- **📖 使用手冊**：內建使用手冊，可於主頁工具列點擊書本圖示開啟查閱 (v1.3)。

### 🏸 開團與收費管理 (v1.5)

- **多球隊 (Club) 子系統**：單一帳號 (Group) 下可建立多個獨立球隊，每個球隊具備專屬 PIN 碼（PBKDF2 安全雜湊）與負責人，確保各隊資料獨立。
- **活動財務與利潤試算**：可記錄單場打球之場租、時數及用球成本（可手動輸入或依指定球種進行 FIFO 自動試算），結合出席收費，一鍵結算當場總營業額與即時盈虧。
- **LINE 報名訊息解析**：支援直接貼上 LINE 群組報名接龍訊息，系統自動智慧擷取活動時間、場地與正選出席者名單，大幅簡化建檔流程。
- **出席與繳費追蹤**：管理單場出席人員名單，支援設定「統一費用」、「一鍵全繳」及「特定人員剔除收費（如主揪免費）」，加速對帳效率。
...
- `clubs`, `badminton_events`, `event_attendees`：開團、活動與收費紀錄核心 table (v1.5)
...
- 🎨 **UI 重構**：主頁頂部 Header 改為 sticky Toolbar，固定於畫面頂端，整合品牌名稱、球團名稱與所有操作按鈕。
- ✨ **優化**：工具列按鈕統一改為圖示樣式，搭配 Radix UI Tooltip（hover 顯示功能名稱）。
- ✨ **優化**：結算試算的球種與領取人篩選，升級為多選勾選下拉（支援「全部」快速全選/全清）。

### v1.5 (2026-04-01)

- ✨ **重磅新增**：全新「開團紀錄」模組上線！支援主帳號 (Group) 建立多個子球隊 (Club)，並具備獨立 PIN 碼安全機制。
- ✨ **新增**：單場活動財務紀錄，統整場地費與用球成本（手動 / FIFO 自動試算），即時計算活動利潤盈虧。
- ✨ **新增**：智能 LINE 報名訊息解析，直接貼上文字自動建立出席名單與場地時間。
- ✨ **新增**：出席與繳費追蹤系統，支援快速設定統一費用、全繳按鈕與免收費人員標記。
- 🔒 **安全**：API 操作加入 Club 權限與 PIN 隔離驗證。
